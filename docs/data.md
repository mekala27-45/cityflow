...
92,634,577 source rows were read from 126 files,
covering 42 months from July 2021 to
December 2024, across 3 services and
5 schema vintages. 92,208,701 rows landed in the
warehouse. 425,876 were quarantined, which is
0.46 percent of what was read. The fact table holds
92,208,701 rows.
...
this repository right now, that is `synthetic`. It is derived from the
...
files. The seed is `20260916`, fixed, so the same window
...
| --- | --- |
| `non_positive_distance` | 0.310 percent of rows |
| `passenger_count_zero` | 0.210 percent |
| `non_positive_fare` | 0.055 percent |
| `implausible_speed` | 0.042 percent |
| `timestamp_out_of_period` | 0.012 percent |
| `dropoff_before_pickup` | 0.008 percent |
| `exact_duplicate` | 0.004 percent |
| `unknown_zone` (kept, not quarantined) | 1.460 percent |
...
volume on 2024-08-12, so that the changepoint detector has
something real to find and the annotation on the chart can be checked against a
known answer. The step is -3.0 percent on yellow,
-1.0 percent on green, and
-12.0 percent on for hire vehicles, which is the arm
...
pickup zone is East Williamsburg, and 23 zones
...
| `duration_too_short` | implausibly brief trips | under 10 seconds | below this it is a meter toggled on and off, not a trip. Low enough that a genuine short hop survives. |
| `duration_too_long` | implausibly long trips | over 12 hours | a meter left running. Above this the duration is measuring a driver's shift, not a journey. |
...
| `distance_too_long` | trips beyond the service area | over 200 miles | beyond the furthest plausible destination for a metered trip originating in the service area. Deliberately generous, so a genuine long run to a neighbouring state survives. |
...
| `fare_too_large` | fare above the ceiling | over 2,000 dollars | a data entry error. Set well above any legitimate flat rate or long run plus tolls. |
| `implausible_speed` | distance over duration above the ceiling | over 80 mph | above this is a record error, not a fast driver: it exceeds any sustained speed achievable on the streets, bridges and parkways in the service area, while leaving a legitimate airport run at highway speed alone. |
| `passenger_count_invalid` | zero, negative, or above the fleet maximum | over 9 passengers | the largest licensed vehicle in the fleet. Null is deliberately left alone: the for hire files do not record passenger count at all, and not recording a value is not the same as recording nonsense. |
...
column sum to the headline 0.4597 percent exactly. That
...
9 fired over this window. The ones that never fired
are `null_timestamp`,
...
On this build, 3,402,805 trips were settled in cash, which is
26.7 percent of yellow and green trips. If you
average `tip_amount / fare_amount` over all of them, you get
14.15 percent. If you average it only over the trips whose
payment channel actually records a tip, you get 20.35
percent. The correct figure is 43.9 percent
...
with no `else`. 3,887,107 rows carry a null `tip_pct` for that
...
On this build they carry 1,574,148 trips, which is
1.71 percent of the fact table. Those trips are real.
...
It holds 263 polygon records covering
260 distinct zone ids. 2 zones are
split across several records, accounting for
5 of those records between them: Corona is two
polygons, and the three harbour islands are three.
3 ids that the trip records can and do reference
...
265 rows, which is every id the TLC publishes, while
`zones.geojson` carries 260 features, one per id that has a
shape to draw.
...
source period, and reports 54 service months in
which the column is entirely null. The first period in which it carries a value
is January 2022, which is the month the column appears
...
December 2024, which is the last month of the window,
...
July 2021 to December 2024 is legitimately all null. A
...
0 service months, which is none of them.
`cbd_congestion_fee`
arrives at the `*_cbd` boundaries, which follow this window, so it is entirely
null in all 126 service months here, which is every
...
217 seconds in ingest across every month.
