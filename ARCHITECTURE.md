...
95 MB per file budget, and GitHub refuses to serve a file
...
day type by month by service. It is 929,826 rows and
25.03 MB, which is the whole of
92,208,701 trip rows reduced to the grain the interface can actually
...
manifest carries the set: this build is `synthetic`, across
126 source files, 42 months, 3
services and 5 schema vintages.
...
TypeScript. dbt builds 14 models and runs 150 data
tests over them, and declares 6 exposures, one per dashboard
...
4 of the 12 panel queries carry a
...
95 MB and `shipped_total_budget_mb` is
400 MB, both in `config/cityflow.yml`; `build_all` raises
...
- The trip grain extract covers one month, and yellow and green only. It is
  303,524 rows and
  5.93 MB. A month of for hire records does not
...
| dbt tests | `make transform` | the warehouse | 150 data tests, including the singular assertions for the zone join fan out and the tip nullability rules |
...
| reconcile | `make reconcile` | warehouse against shipped layer | any of the 16 metrics whose two expressions disagree, at four grains |
